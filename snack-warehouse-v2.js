(()=>{
 ...
 const WAREHOUSE_ROUTE_RATIO=.25;
 ...
 function withVisibleEnemiesFrom(x,y,callback){
  const allEnemies=enemies;enemies=allEnemies.filter(enemy=>!enemyBlockedFrom(x,y,enemy));
 ...
 routePathById=function(routeId){return routeId==="warehouse"?warehouseRoute:originalRoutePathById(routeId)};
 ...
 window.SNACK_WAREHOUSE=Object.freeze({
  unlockWave:WAREHOUSE_ROUTE_UNLOCK_WAVE,routeRatio:WAREHOUSE_ROUTE_RATIO,position:{...warehouse},route:warehouseRoute.map(point=>[...point]),buildSpots:warehouseBuildSpots.map(point=>[...point]),barricade:{startWave:warehouseBarricade.startWave,points:warehouseBarricade.points.map(point=>[...point])},blocksShot:(x1,y1,x2,y2)=>lineBlocked(x1,y1,x2,y2)
 });
})();
