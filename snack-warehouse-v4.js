(()=>{
 ...
 const branchBuildSpots=[
  [1100,445],[1015,605],[910,485],[825,545],
  [735,705],[635,530],[540,680],[445,575]
 ];

 const warehouse={x:132,y:235,doorX:198,doorY:242};
 const warehouseRoute=[
  [953,100],[940,140],[910,175],[870,200],[825,212],[780,205],
  [740,185],[700,170],[660,176],[625,198],[605,228],[596,258],
  [570,278],[530,286],[490,278],[455,255],[425,230],[390,218],
  [355,226],[330,250],[316,278],[290,300],[260,296],[230,276],
  [208,254],[198,242]
 ];
 ...
 const warehouseBuildSpots=[
  [875,285],[790,285],[705,255],[620,315],
  [520,195],[445,315],[350,175],[265,340]
 ];
 const warehouseBarricade={
  label:"倉庫ルート",
  startWave:WAREHOUSE_ROUTE_UNLOCK_WAVE,
  labelPosition:[680,402],
  points:[[930,315],[855,322],[780,330],[705,338],[630,348],[555,358],[480,368],[405,374],[330,370],[265,356],[210,334]]
 };
 const warehouseBlockedKinds=new Set(["mainBoss","armorBoss","midBoss","golden"]);
 const routeBuildSpots=[...branchBuildSpots,...warehouseBuildSpots];
 let routeSpotStart=-1;
 ...
 function syncRouteBuildSpots(){
  if(warehouseModeActive()){
   if(routeSpotStart>=0)return;
   routeSpotStart=spots.length;
   routeBuildSpots.forEach(point=>spots.push([...point]));
   return;
  }
  if(routeSpotStart<0)return;
  const occupied=towers?.some(tower=>tower.spot>=routeSpotStart&&tower.spot<routeSpotStart+routeBuildSpots.length);
  if(occupied)return;
  spots.splice(routeSpotStart,routeBuildSpots.length);
  routeSpotStart=-1;
 }
 ...
 function drawClosedWarehouseApproach(){
  ...
  ctx.save();ctx.translate(268,296);ctx.globalAlpha=.8;ctx.strokeStyle="#654522";ctx.fillStyle="#a8753d";ctx.lineWidth=3;ctx.beginPath();
 ...
 function drawOpenWarehouseRoute(){
  ...
  [[880,186],[605,238],[360,228]].forEach(([x,y])=>{ctx.fillText("🍃",x-18,y-12);ctx.fillText("🪵",x+18,y-8)});
 ...
 function drawBarricadeGuide(){
  ...
  strokeRoute(warehouseBarricade.points,"#f2d69c",3,.36,[8,9]);
  ctx.save();ctx.textAlign="center";ctx.font="bold 11px 'Mochiy Pop One',sans-serif";ctx.fillStyle="rgba(255,239,190,.84)";ctx.strokeStyle="rgba(62,39,20,.82)";ctx.lineWidth=3;
  ctx.strokeText("倉庫側",680,322);ctx.fillText("倉庫側",680,322);
  ctx.strokeText("中央側",680,387);ctx.fillText("中央側",680,387);
  ctx.restore();
 }
 ...
 function previewTowerSource(tower=null){
  if(tower){
   if(!(tower.damage>0||tower.kind==="peace"))return null;
   return{x:tower.x,y:tower.y-35,range:effectiveRange(tower),kind:tower.kind};
  }
  ...
  return{x:spot[0],y:spot[1]-35,range,kind:selectedType};
 }
 ...
 function drawWarehouseVisibilityPreview(tower=null){
  ...
  for(let index=0;index<warehouseRoute.length-1;index++){
   ...
   const isBlocked=source.kind!=="star"&&lineBlocked(source.x,source.y,mx,my);
   if(isBlocked)blocked++;else visible++;
   ctx.strokeStyle=isBlocked?"rgba(239,103,91,.95)":"rgba(255,225,92,.96)";
   ctx.shadowColor=isBlocked?"#d6554c":"#ffe05b";ctx.shadowBlur=9;ctx.lineWidth=9;ctx.setLineDash(isBlocked?[7,6]:[]);ctx.beginPath();ctx.moveTo(a[0],a[1]);ctx.lineTo(b[0],b[1]);ctx.stroke();
  }
  ctx.setLineDash([]);ctx.shadowBlur=0;
  if(visible||blocked){
   const text=source.kind==="star"?"★ スター：防壁を無視":blocked?"黄色：攻撃可　赤：防壁で遮断":"黄色：攻撃可能";
   const x=Math.max(155,Math.min(1045,source.x)),y=Math.max(92,Math.min(700,source.y+72));
   ctx.font="bold 12px 'Mochiy Pop One',sans-serif";ctx.textAlign="center";const width=ctx.measureText(text).width+20;ctx.fillStyle="rgba(44,31,20,.82)";ctx.strokeStyle="rgba(255,238,185,.8)";ctx.lineWidth=1.5;ctx.beginPath();ctx.roundRect(x-width/2,y-15,width,25,8);ctx.fill();ctx.stroke();ctx.fillStyle="#fff2bd";ctx.fillText(text,x,y+3);
  }
  ctx.restore();
 }
 ...
 function spawnSnackFlight(){
  ...
  snackFlights.push({amount,t:0,x0:610+Math.random()*170,y0:40+Math.random()*45,curveX:360+Math.random()*80,curveY:40+Math.random()*55,phase:Math.random()*Math.PI*2});
 }
 ...
 activeEnemyPaths=function(){
  ...
  if(routeBranchActive)paths.push(branchSnakePath);
  if(warehouseRouteOpen())paths.push(warehouseRoute);
  return paths;
 };
 ...
 window.SNACK_WAREHOUSE=Object.freeze({
  ...
  position:{...warehouse},
  branchRoute:branchSnakePath.map(point=>[...point]),
  route:warehouseRoute.map(point=>[...point]),
  branchBuildSpots:branchBuildSpots.map(point=>[...point]),
